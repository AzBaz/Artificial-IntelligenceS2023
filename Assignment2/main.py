...
def get_safe_moves(game_state, snake, maximizingPlayer):
...
def minimax(game_state, depth, maximizingPlayer):
  ...
  if depth == 0:
    return get_heuristic(game_state)

  if maximizingPlayer:
    #get safe moves for max player (you)
    safe_moves = get_safe_moves(game_state, game_state["you"], True)
    ...
    if snake_index != None:
      safe_moves = get_safe_moves(game_state, game_state["board"]["snakes"][snake_index], False)
    ...
    for move_choice in safe_moves:
      ...
      if type(possibleTuple) is tuple:
        value, bestMove = min([(value, bestMove), (possibleTuple[0], move_choice)], key=itemgetter(0))
      else:
        value, bestMove = min([(value, bestMove), (possibleTuple, move_choice)], key=itemgetter(0))
    return (value, bestMove)
...
def move(game_state: typing.Dict) -> typing.Dict:
  next_move = None
  try:
    heuristic_value, next_move = minimax(game_state, 6, True)
  except:
    print("something went horribly wrong. going down")
    next_move = "down"
